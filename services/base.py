...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get(self, db_session: Session, id: int, id_name="id") -> Optional[ModelType]:
        return (
            db_session.query(self.model)
            .filter(getattr(self.model, id_name) == id)
            .first()
        )

    def get_all(self, db_session: Session) -> Optional[ModelType]:
